...
class UnifiedStaticCache(Cache):
    
    def __init__(self, caches: List[StaticCache] = None):
        self.caches: List[StaticCache] = caches if caches is not None else []
        super().__init__()
    ...
    def update(
            ...
            for i, cache in enumerate(self.caches):
                # Ensure key_states and value_states are on the same device as the cache
                device = cache.key_cache[layer_idx].device
                k, v = cache.update(key_states[i:i+1].to(device), value_states[i:i+1].to(device), layer_idx, cache_kwargs)
    ...
    def get_max_cache_shape(self):
        return max([cache.get_max_cache_shape() for cache in self.caches])